...
Веб-интерфейс и бот для анализа изображений растений: сегментация (корень, стебель, лист), площади и длины в мм/см, калибровка по шахматной доске.
...
- Модель YOLO в папке `all_data/yolo_weights/plants_optimized_seg/weights/best.pt`
...
Дальше: загрузка фото растения → при необходимости калибровка и настройки → «Запустить анализ» → просмотр результата и таблицы.
...
В боте: отправь фото растения для анализа; команда `/calibration` — выбор режима калибровки (стандартная / по фото), `/calibrate` — калибровка по фото доски.

---

## Калибровка (перевод в см)

Масштаб можно задать двумя способами:

| Режим | Описание |
|--------|-----------|
| **Стандартная** | Фиксированное значение (по умолчанию или переменная окружения `MM_PER_PX`). Фото не нужны. |
| **По фото** | Загружаешь фото шахматной доски (клетка **10×10 мм**). Поддерживаются разные сетки: 5×8, 7×4, 5×3 и др. |

- **Веб:** блок «Калибровка» → переключатель «Стандартная» / «По фото». В режиме «По фото» — выбор файла и кнопка «Загрузить фото доски и применить».
- **Бот:** `/calibration` — выбор режима; в режиме «По фото» отправь `/calibrate`, затем фото доски.

Файл `calibration.json` в корне проекта хранит режим и значение масштаба.
...
│   └── bot.py           # Telegram-бот
├── all_data/
│   └── yolo_weights/... # Веса модели (должны быть установлены)
├── runs/                # Результаты анализов (создаётся при первом запуске)
├── calibration.json     # Режим и масштаб калибровки (создаётся при калибровке)
├── requirements.txt
└── README.md
```
